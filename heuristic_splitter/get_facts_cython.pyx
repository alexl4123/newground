...
def get_facts_from_file_handle(f):
    ...
    cdef bytearray current_rule_bytearray_2 = bytearray(current_rule_size)
    cdef int[100] current_rule_bytearray 

    cdef int current_fact_id_function_size = 20
    cdef int current_fact_id_list_head = 0
    cdef bytearray current_fact_id_bytearray_2 = bytearray(current_fact_id_function_size)
    cdef int[20] current_fact_id_bytearray
            ...
            if is_fact is True:
                    ...
                    if in_fact_id is True: # In fact ID: I.e. in abcd(..,..) -> in abcd
                        if char_is_valid_ID_char(cur_char): # In fact ID: I.e., in abcd(..,..) -> in bcd (not in a)
                            ...
                            for index in range(current_fact_id_list_head):
                                current_fact_id_bytearray_2[index] = current_fact_id_bytearray[index]

                            if PyDict_Contains(facts, PyUnicode_FromString(current_fact_id_bytearray_2)):
                            ...
                            if current_fact_head_in_dict is False:
                                ...
                                PyDict_SetItem(facts, PyUnicode_FromString(current_fact_id_bytearray_2), True)
                                PyDict_SetItem(facts_heads, PyUnicode_FromString(current_fact_id_bytearray_2), True)
                                PyDict_SetItem(all_heads, PyUnicode_FromString(current_fact_id_bytearray_2), 0)

                                current_fact_id_bytearray_2 = bytearray(current_fact_id_function_size)
                        ...
                        if in_string is False:
                            ...
                            if cur_char == quotation_mark_char:
                                # TODO -> Add support for escaping quotation marks(?)
                                in_string = False
                        ...
                        if current_fact_head_in_dict is False:
                            ...
                            PyDict_SetItem(facts_heads, PyUnicode_FromString(current_fact_id_bytearray_2), True)
                            PyDict_SetItem(all_heads, PyUnicode_FromString(current_fact_id_bytearray_2), current_fact_number_terms)
                        ...
                        for index in range(current_rule_list_head):
                            current_rule_bytearray_2[index] = current_rule_bytearray[index]

                        PyDict_SetItem(facts, PyUnicode_FromString(current_rule_bytearray_2), True)
                ...
                if in_string is False:
                    if cur_char != quotation_mark_char and cur_char != dot_char:
                        ...
                        PyList_Append(other_rules, PyUnicode_FromString(current_rule_bytearray_2))

                        current_rule_bytearray_2 = bytearray(current_rule_size)
                        current_fact_id_bytearray_2 = bytearray(current_fact_id_function_size)
